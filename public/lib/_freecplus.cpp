...
void UpdateStr(char *str, const char *str1, const char *str2, bool bloop)
{
    ...
    if ((bloop == true) && (strstr(str2, str1) > 0))
        return;
...
bool MatchStr(const string str, const string rules)
{
    ...
    for (ii = 0; ii < CmdStr.CmdCount(); ii++)
    {
        ...
        for (jj = 0; jj < CmdSubStr.CmdCount(); jj++)
        {
            ...
            {
                if (strcmp(strFileName.c_str() + strFileName.size() - CmdSubStr.m_vCmdStr[jj].size(), CmdSubStr.m_vCmdStr[jj].c_str()) != 0)
                    break;
            }
...
//// 把内容写入日志文件，fmt是可变参数，使用方法与printf库函数相同。
//// Write方法会写入当前的时间，WriteEx方法不写时间。
//// 与Write方法不同的是他会同时输出写入log日志的内容，建议用于初写时的调试
bool CLogFile::Write_P(const char *fmt, ...)
{
    if (m_tracefp == 0)
        return false;
    ...
    LocalTime(strtime);
    char strprintf[256];
    memset(strprintf, 0, sizeof(strprintf));

    va_list ap;
    va_start(ap, fmt);
    vsprintf(strprintf, fmt, ap);
    va_end(ap);

    fprintf(m_tracefp, "%s %s", strtime, strprintf);
    printf("%s", strprintf);
...
double Evaluate(char expr[])
{
    ...
    while ((ch = expr[i]) != 0)
    {
        if (Evaluate_checknumber(ch))
        {
            numbuf[nbi++] = ch;
            if (!Evaluate_checknumber(expr[i + 1]))
            {
                numbuf[nbi] = 0;
                nbi = 0;
        ...
        {
            while ((osi > 0) && (Evaluate_precedence(ch) <= Evaluate_precedence(operators[osi - 1])))
            {
                numbers[nsi - 2] = Evaluate_calculate(operators[osi - 1], numbers[nsi - 2], numbers[nsi - 1]);
                osi--;
                nsi--;
            }
            operators[osi++] = ch;
...
char Evaluate_checknumber(char ch)
{
    if ((ch >= '0' && ch <= '9') || ch == '.')
        return 1;
    else
...
int Evaluate_precedence(char ch)
{
    int Evaluate_precedence;
    switch (ch)
    {
    case '+':
    case '-':
        Evaluate_precedence = 0;
        break;
    case '*':
    case '/':
        Evaluate_precedence = 1;
        break;
    case '^':
        Evaluate_precedence = 2;
    }
    return Evaluate_precedence;
}

double Evaluate_calculate(char moperator, double num1, double num2)
{
    double result;
    switch (moperator)
    {
...
CRand::CRand(double DouInit)
{
    ...
    {
        m_Result = (int)(DouInit * 10000.0);
    }
}

CRand::CRand(int in_Min, int in_Max)
{
    if (in_Min > in_Max)
    {
        this->m_Max = in_Min;
...
bool CRand::GetRandResult()
{
    //这里只能用微秒计时，不然取值都会一样
    struct timeval st_time;
    gettimeofday(&st_time, 0);

    srand(st_time.tv_usec);

    if ((rand() % 10000 + 1) <= m_Result)
        return true;
    else
        return false;
}

unsigned int CRand::GetRandNum()
{
    //这里只能用微秒计时，不然取值都会一样
    struct timeval st_time;
    gettimeofday(&st_time, 0);
    ...
    int TempNum=(rand() % (m_Max - m_Min + 1)) + m_Min;
