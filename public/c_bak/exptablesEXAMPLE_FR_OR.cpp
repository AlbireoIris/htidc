...
bool LoadMaxKeyid()
{
  ...
  sqlstatement stmt(&conn);
  stmt.prepare("select keyid from T_SYSDATA where taskname=:1");
  ...
  if (stmt.next() != 0)
  {
    stmt.prepare("insert into T_SYSDATA(taskname,keyid) values(:1,0)");
    stmt.bindin(1,starg.taskname,50);
...
bool UptMaxKeyid()
{
  ...
  sqlstatement stmt(&conn);
  stmt.prepare("update T_SYSDATA set keyid=:1 where taskname=:2");
