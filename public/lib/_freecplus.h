...
public:
  ...
  // Write_P和Write方法不同的是他会同时输出写入log日志的内容，建议用于初写时的调试
  bool Write(const char *fmt, ...);
  bool Write_P(const char *fmt, ...);
...
//这个函数用来计算简单的字符串类型的表达式。
double Evaluate(char[]);

//这三个函数是Evaluate函数的依赖函数。
int Evaluate_precedence(char);
double Evaluate_calculate(char, double, double);
char Evaluate_checknumber(char);
